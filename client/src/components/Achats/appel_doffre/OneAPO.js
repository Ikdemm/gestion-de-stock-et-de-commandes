...
if(tabProduits&&pdt){
  return (
    ...
    <td  >{  moment(props.apo.dateAPO).locale('fr').format('LL') }</td>
        <td  >{props.apo.ref}</td>
        <td  >{ moment(props.apo.dateLimiteDeReponse).locale('fr').format('LL') }</td>
        <td  >{ moment(props.apo.dateDeCommandePlanifiee).locale('fr').format('LL') } </td>
        <td  >
        <button className='btn btn-outline-dark'  onClick={handleOpen}> <AiTwotoneEye /></button> 

          </td>
     
        <td  > 
        <button className='btn btn-danger' onClick={removeC}><BsTrash></BsTrash></button>
        </td>
    </tr>  
    <Modal
            keepMounted
